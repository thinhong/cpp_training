...
std::vector<std::weak_ptr<Model>> FullModel::getModelsWithGroup(std::vector<std::string> groupToGet) {
...
void FullModel::connectModels() {
    ...
    for (auto& model: models) {
        ...
        for (auto linkedModel: model->getLinkedModels()) {
            ...
            model->addNewLinkedContactProb(getContactProbByComparingPairs(classPair));
        }
        ...
        for (size_t i {0}; i <= lastIndex - 1; ++i) {
            ...
            for (auto subsetModel: subsetModels) {
                ...
                model->updateLinkedContactProb(getContactProbByComparingPairs(classPair), k);
            }
        }
    }
}